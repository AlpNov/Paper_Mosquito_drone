...
\end{figure}
  ...
  Tests with an oscilloscope show that the screen voltage monitoring circuit shows a deflection of about 710 mV when the mosquito contacts the screen.  The recovery time is approximately 160 ms.  Fig.~\ref{fig:BugZapTrace} shows the time sequence.  The voltage recovery can be modeled with an exponential fit.
 \begin{align} \label{eq:BugZapFit}
v=V_{ss}(1-ae^{-13.09t})
\end{align}

The fit in \eqref{eq:BugZapFit} gives us a time constant of $13.09/s$.
  
  \todo{
  how many mAh to keep an LxL screen charged?
  How many mAh to kill one mosquito?}
  
                \begin{figure}
\centering
\begin{overpic}[width=1.0\columnwidth]{BugZapTrace.pdf}\end{overpic}
\caption{\label{fig:BugZapTrace}
  Voltage trace as a mosquito contacts the bug-zapping screen.  It causes a voltage deflection of 710 mV that recovers in 160 ms.
  } 
\end{figure}
